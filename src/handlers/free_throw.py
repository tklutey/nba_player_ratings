...
def back_iterate_ft(row, df_canonical):
    ...
    while cur_row['Event_Msg_Type'] != definitions.FOUL_EVENT_MSG_TYPE:
        if cur_row['Event_Msg_Type'] == definitions.FT_EVENT_MSG_TYPE:
...
def get_prev_row(row, df):
    return df.loc[(df['Game_id'] == row['Game_id'])
                  & (df['Canonical_Game_Event_Num']
                     == row['Canonical_Game_Event_Num'] - 1)
                  ].iloc[0].copy()


def get_last_ft_events(df):
    df = df.loc[df['Event_Msg_Type'] == definitions.FT_EVENT_MSG_TYPE].copy()
    df = df.loc[
        (df['Action_Type'] == definitions.FT_1_OF_1_ACTION_TYPE)
        | (df['Action_Type'] == definitions.FT_2_OF_2_ACTION_TYPE)
        | (df['Action_Type'] == definitions.FT_3_OF_3_ACTION_TYPE)
        | (df['Action_Type'] == definitions.FT_TECHNICAL_ACTION_TYPE)
        | (df['Action_Type'] == definitions.FT_CLEAR_PATH_ACTION_TYPE)
        | (df['Action_Type'] == definitions.FT_FLAGRANT_2_OF_2_ACTION_TYPE)
        | (df['Action_Type'] == definitions.FT_FLAGRANT_1_OF_1_ACTION_TYPE)
        | (df['Action_Type'] == definitions.FT_TECHNICAL_2_OF_2_ACTION_TYPE)
        | (df['Action_Type'] == definitions.FT_CLEAR_PATH_2_OF_2_ACTION_TYPE)
        | (df['Action_Type'] == definitions.FT_FLAGRANT_3_OF_3_ACTION_TYPE)
                ].copy()
