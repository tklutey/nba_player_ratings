...
## Columns: Period, Player 1 - 5, team_id, game_id
def main():
...
def process_substitution(df_sub, df_lineups):
    for i in range(0, len(df_sub)):
        ...
        previous_lineup = get_previous_lineup(df_lineups, game_id, team_id, period, event_id)
...
def get_previous_lineup(df_lineups, game_id, team_id, period, event_id):
    ...
    df_lineups = df_lineups.loc[df_lineups['Start_Canonical_Game_Event_Num'] == previous_event_id]
...
def get_lineup_row(df_decomposed_lineup):
    ...
    canonical_event_id = get_canonical_event_id(df_composed_lineup['Game_id'][0], df_composed_lineup['Period'][0])
    df_composed_lineup['Start_Canonical_Game_Event_Num'] = pd.Series(canonical_event_id)
    counter = 1
...
def get_canonical_event_id(game_id, period):
    ...
    df = df.loc[df['Event_Msg_Type'] == PERIOD_START_EVENT_MSG_TYPE]
    df = df.loc[df['Action_Type'] == PERIOD_START_ACTION_TYPE]
