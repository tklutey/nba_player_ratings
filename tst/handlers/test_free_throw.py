...
class FTTests(unittest.TestCase):
    ...
    def test_filter_and_generate_columns(self):
        ...
        df_output = free_throw.generate_columns(df_filter, df_canonical)[['Canonical_Game_Event_Num', 'Game_id',
                                                                          'Event_Msg_Type', 'Action_Type',
                                                                          'Actualized_points',
                                                                          'Actualized_Canonical_Game_Event_Num']]\
            .reset_index(drop=True)
        df_expected = pd.DataFrame(
            {'Canonical_Game_Event_Num': [3], 'Game_id': [1],
             'Event_Msg_Type': [FT_EVENT_MSG_TYPE], 'Action_Type': [FT_2_OF_2_ACTION_TYPE], 'Actualized_points': [2],
             'Actualized_Canonical_Game_Event_Num': [1]})
